 ...
 */
export const run =
  <Args extends any[], State, Result>(
    fn: (...args: Args) => Result,
    state: (...args: Args) => State,
    until: (state: State, res: Result) => boolean,
  ): ((...args: Args) => Result) =>
  (...args) => {
    for (let res: any, s = state(...args); ; )
      if (until(s, (res = fn(...args)))) return res;
  };
...
export const runAsync =
  ...
  async (...args) => {
    for (let res, s = state(...args); ; )
      if (await until(s, (res = await fn(...args)))) return res as any;
  };
...
export const untilAsync =
  ...
  async (...args) => {
    for (let res; ; )
      if (await pass((res = await fn(...args)))) return res as any;
  };
